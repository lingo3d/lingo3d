...
const Controls: React.FC<{
  ...
  return (
    <>
      <Keyboard
        onKeyPress={(key) => {
          if (key === "w") camera.moveForward(10)
          if (key === "s") camera.moveForward(-10)
          if (key === "a") camera.moveRight(-10)
          if (key === "d") camera.moveRight(10)
        }}
        onKeyDown={(key) => {
          if (key === " ") {
...
const App = () => {
  ...
  const fire = useCallback(
    (bullet: types.Cube | null) => bullet?.applyLocalImpulse(0, 0, -1),
    []
  )
    ...
    <>
      <World bloom bloomStrength={0.5} bloomThreshold={0.9}>
        ...
        <Cube
          ...
          physics
          color="red"
          rotationY={rotationKeyframes}
          onMouseOver={() => setScale(2)}
  ...
  return (
    <>
      ...
      <div style={{ position: "absolute", marginTop: 100 }}>
        <button onClick={pushUserData}>add user</button>
        <button onClick={pullUserData}>remove user</button>
      </div>
      <Stats mode="fps" />
    </>
  )
